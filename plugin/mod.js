    ...
    function RezkaCategory(category) {
        ...
        comp.card = function(item) {
            ...
            card.on('hover:focus', function() {
                ...
                // ВАЖНО: Обновляем скролл при фокусе (из старого кода)
        ...
        comp.start = function() {
            ...
            // ВНЕДРЕНИЕ УПРАВЛЕНИЯ ПУЛЬТОМ ИЗ СТАРОГО КОДА
            // Используем имя 'rezka', чтобы не конфликтовать с системным 'content'
            Lampa.Controller.add('rezka', {
                toggle: function() {
                    Lampa.Controller.collectionSet(comp.html);
                    Lampa.Controller.collectionFocus(last_item, comp.html);
                },
                // --- Навигация (из старого кода) ---
                left: function() {
                ...
                up: function() {
                    if (Navigator.canmove('up')) {
                        Navigator.move('up');
                        if (scroll) scroll.minus(); // Скроллинг
                    } else {
                ...
                down: function() {
                    if (Navigator.canmove('down')) {
                        Navigator.move('down');
                        if (scroll) scroll.plus(); // Скроллинг
                    }
                },
                // -----------------------------------
                back: function() {
